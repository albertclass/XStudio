...
protected:
	...
	struct COMMON_API SavedPosMap
	{
		...
		void AllocMapTable( ) { pTable = XGC_NEW SavedPos*[SPM_SIZE]; for( int n = 0; n<SPM_SIZE; ++n ) pTable[n] = NULL; };
		int Hash( const char* szName ) { int n=0; while (*szName) n += *szName++; return n % SPM_SIZE; };
		SavedPos** pTable;
	};
	SavedPosMap m_mapSavedPos;

	struct COMMON_API PosArray
	{
		PosArray() 
		{ 
			Clear(); 
	...
	protected:
		...
		{ 
			NodePos* pNNew = XGC_NEW NodePos[nNewSize];
			Copy(pNNew); 
			nSize=nNewSize; 
		}
...
//---------------------------------------------------//
// [24/2/2014 baomin]
// Description:	获取xml元素的数字类型的属性值 
//---------------------------------------------------//
template < class T, typename std::enable_if< std::is_signed< T >::value, xgc_bool >::type = true >
inline xgc_bool GetXMLAttribInteger( const CMarkupSTL& xml, const char* szAttrib, T& _Value )
{
	std::string strRet = xml.GetAttrib( szAttrib );
	if( strRet.empty() )
	{
		return false;
	}
	_Value = (T)strtol( strRet.c_str(), xgc_nullptr, 10 );
	return true;
}

template < class T, typename std::enable_if< std::is_unsigned< T >::value, xgc_bool >::type = true >
inline xgc_bool GetXMLAttribInteger( const CMarkupSTL& xml, const char* szAttrib, T& _Value )
{
	std::string strRet = xml.GetAttrib( szAttrib );
	if( strRet.empty() )
	...
	_Value = (T)strtoul( strRet.c_str(), xgc_nullptr, 10 );
	return true;
}

template <>
inline xgc_bool GetXMLAttribInteger( const CMarkupSTL& xml, const char* szAttrib, xgc_bool &_Value )
{
	std::string strRet = xml.GetAttrib( szAttrib );
	if( strRet.empty() )
		return false;

	_Value = ( stricmp( "true", strRet.c_str() ) == 0 );
	return true;
}

template <>
inline xgc_bool GetXMLAttribInteger( const CMarkupSTL& xml, const char* szAttrib, xgc_real32 &_Value )
{
	std::string strRet = xml.GetAttrib( szAttrib );
	if( strRet.empty() )
	{
	...
	_Value = strtof( strRet.c_str(), xgc_nullptr );
	return true;
}

template <>
inline xgc_bool GetXMLAttribInteger( const CMarkupSTL& xml, const char* szAttrib, xgc_real64 &_Value )
{
	std::string strRet = xml.GetAttrib( szAttrib );
	if( strRet.empty() )
	{
	...
	_Value = strtod( strRet.c_str(), xgc_nullptr );
	return true;
}
...
/// @param xml XML文档对象
/// @param szAttrib 属性名
/// @param szBuffer 缓冲区地址
/// @param nSize	缓冲区长度
///
COMMON_API xgc_bool GetXMLAttribString( const CMarkupSTL& xml, const char* szAttrib, char* szBuffer, size_t nSize );

///
/// [4/16/2014 guqiwei.weir]
/// 从xml节点中获得属性信息，如果没有设置则
/// 使用默认值, 即节点可以不设置此属性
/// @param xml xml节点
/// @param attrName 节点属性名
/// @param value 属性值
/// @return 找到节点并能正确返回为true，否则false
/// @exception 类型转换失败向上抛出invalid_argument异常
///
template < typename _T >
inline xgc_bool GetXMLAttrValue( const CMarkupSTL& node, const std::string& attrName, _T& value )
{
	std::string strValue = node.GetAttrib( attrName.c_str() );
	if( strValue.empty() )
	{
		return false; // 使用默认值
	}
...
template < >
inline xgc_bool GetXMLAttrValue( const CMarkupSTL& node, const std::string& attrName, std::string& value )
{
	std::string strValue = node.GetAttrib( attrName.c_str() );
	if( strValue.empty() )
		return false; // 使用默认值
...
//-----------------------------------//
template < typename _T >
inline _T GetXMLAttrValue( const CMarkupSTL& node, const std::string& attrName )
{
	_T value;
	std::string strValue = node.GetAttrib( attrName.c_str() );
	if( strValue.empty() )
...
template < >
inline xgc_string GetXMLAttrValue( const CMarkupSTL& node, const xgc_string& attrName )
{
	xgc_string strValue = node.GetAttrib( attrName.c_str() );
	if( strValue.empty() )
	{
		throw std::invalid_argument( "Wrong Tag" );
	}
...
//-----------------------------------//
template < typename _T >
inline xgc_bool GetXMLNodeValue( const CMarkupSTL& node, _T& value )
{
	xgc_string strValue = node.GetData();
	if( strValue.empty() )
	{
		return false; // 使用默认值
	}
...
template < >
inline xgc_bool GetXMLNodeValue( const CMarkupSTL& node, xgc_string& strValue )
{
	strValue = node.GetData();
	if( strValue.empty() )
	{
		return false; // 使用默认值
	}
...
//-----------------------------------//
template < typename _T >
inline _T GetXMLNodeValue( const CMarkupSTL& node )
{
	_T value;
	std::string strValue = node.GetData();
	if( strValue.empty() )
	{
...
template < >
inline xgc_string GetXMLNodeValue( const CMarkupSTL& node )
{
	std::string strValue = node.GetData();
	if( strValue.empty() )
	{
