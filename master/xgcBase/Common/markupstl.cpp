...
void CMarkupSTL::operator=( const CMarkupSTL& markup )
{
	...
	if ( m_aPos.nSegs )
	{
		m_aPos.pSegs = (ElemPos**)( XGC_NEW char[m_aPos.nSegs*sizeof( char* )] );
		...
		for ( int nSeg=0; nSeg < m_aPos.nSegs; ++nSeg )
		{
			...
			m_aPos.pSegs[nSeg] = (ElemPos*)( XGC_NEW char[nSegSize*sizeof( ElemPos )] );
	...
	if ( markup.m_mapSavedPos.pTable )
	{
		...
		for ( int nSlot=0; nSlot < SavedPosMap::SPM_SIZE; ++nSlot )
		{
			...
			if ( pCopySavedPos )
			{
				...
				SavedPos* pNewSavedPos = XGC_NEW SavedPos[nCount];
...
bool CMarkupSTL::ReadTextFile( const char* szFileName, string& strDoc, string* pstrError, int* pnFlags )
{
	...
	if ( nFileByteLen )
	{
		char* pszBuffer = XGC_NEW char[nFileByteLen];
		...
		else
		{
			while ( pSavedPos[nOffset].nSavedPosFlags & SavedPosMap::SPM_USED )
			{
				...
				if ( pSavedPos[nOffset].nSavedPosFlags & SavedPosMap::SPM_LAST )
				{
					...
					SavedPos* pNewSavedPos = XGC_NEW SavedPos[nNewSize];
...
bool CMarkupSTL::x_AllocPosArray( int nNewSize /*=0*/ )
{
	...
	if ( m_aPos.GetSize() < nNewSize )
	{
		...
		if ( m_aPos.nSegs <= nNewSeg )
		{
			...
			char* pNewSegments = XGC_NEW char[nNewSegments*sizeof( char* )];
			if ( m_aPos.SegsUsed() )
		...
		{
			char* pNewFirstSeg = XGC_NEW char[nFullSegSize * sizeof( ElemPos )];
			if ( nSegSize )
			{
				// Reallocate
		...
		// New segment
		char* pNewSeg = XGC_NEW char[nNewSegSize * sizeof( ElemPos )];
	...
	while ( 1 )
	{
		...
		else if ( nTypeFound == 0 ) // end tag
		{
			...
			if ( nMatchDepth == 0 )
			{
				...
				if ( m_strError.empty() )
				{
					char* szError = XGC_NEW char[token.Length( ) + 100];
		...
		if ( nMatchDepth || nTypeFound == -2 )
		{
			...
			while ( nDepth > nMatchDepth )
			{
				...
				if ( m_strError.empty() )
				{
					if ( nTypeFound == 0 )
					{
						char* szError = XGC_NEW char[aNodes.Top( ).strMeta.size( ) + token.Length( ) + 100];
					...
					{
						char* szError = XGC_NEW char[aNodes.Top( ).strMeta.size( ) + 100];
...
xgc_bool GetXMLAttribString( const CMarkupSTL& xml, const char* szAttrib, char* szBuffer, size_t nSize )
{
	std::string strValue = xml.GetAttrib( szAttrib );
	if( strValue.empty() )
		return false;
	...
	strcpy_s( szBuffer, nSize, strValue.c_str() );
