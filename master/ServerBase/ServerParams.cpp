...
/// [12/19/2014] create by albert.xu
///
static xgc_bool LoadGlobalConfig( IniFile &ini )
{
	xgc_lpcstr lpSecionName = "ServerParams";
	xgc_uint32 nNewParamCount = 0;

	if( ini.IsExistSection( lpSecionName ) )
	{
		// 存在配置则读取，不存在则略过
		for( xgc_size nIdx = 0; nIdx < ini.GetItemCount( lpSecionName ); ++nIdx )
		{
			xgc_lpcstr lpKey = ini.GetItemName( lpSecionName, nIdx );
			xgc_lpcstr lpVal = ini.GetItemValue( lpSecionName, nIdx, xgc_nullptr );
			...
			{
				// 路径配置可以使用转义字符串
				xgc_char szConfPath[_MAX_PATH] = { 0 };

				ini.TransformValue( szConfPath, lpVal );
				
				xgc_char szIniFile[_MAX_PATH] = { 0 };
				GetNormalPath( szIniFile, _MAX_PATH, szConfPath );
	...
	if( sql_success == res )
	{
		while( movenext( rs ) )
		{
			...
			if( it == gGlobalParams.end() )
			{
				...
				stParam.mask = mask;
				stParam.update = datetime::current_time();
...
xgc_bool SetGlobalParameter( xgc_lpcstr lpKey, xgc_lpcstr lpValue, xgc_uint32 nMask )
{
	...
	if( it == gGlobalParams.end() )
	{
		if( 0 == XGC_CHKMASK( nMask, GLOBAL_PARAM_MASK_CREATE ) )
			return false;
	...
	it->second.update = datetime::current_time();

	if( XGC_CHKMASK( nMask, GLOBAL_PARAM_MASK_NOTIFY ) && gNotifyCallback )
