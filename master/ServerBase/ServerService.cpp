...
#define SVC_LOG( FMT, ... ) FileLog( gServiceLog, FMT, __VA_ARGS__ )
...
int main( int argc, char *argv[] )
{
	...
	if( argc > 1 )
	{
		if( stricmp( argv[1], "-install" ) == 0 )
		{
			...
			lpServiceName = ini.GetItemValue( "ServiceCfg", "ServiceName", xgc_nullptr );
			...
			lpServiceDisp = ini.GetItemValue( "ServiceCfg", "DisplayName", lpServiceName );
			ini.TransformValue( szServiceDisp, lpServiceDisp );
			lpServiceDesc = ini.GetItemValue( "ServiceCfg", "Description", "这个人很懒注册服务的时候都不写服务描述。" );
			ini.TransformValue( szServiceDesc, lpServiceDesc );
		...
		else if( stricmp( argv[1], "-service" ) == 0 )
		{
			...
			SVC_LOG( "service start at %s config file=%s\n",
				datetime::current_time().to_string( szDateTime, sizeof(szDateTime) ),
				argv[2] );
