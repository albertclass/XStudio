...
namespace Json {
	...
	bool Reader::readToken( Token &token )
	{
		...
		case ']':
			...
			break;
		case '<':
			token.type_ = tokenFileBegin;
			ok = readFile();
			break;
		case '>':
			token.type_ = tokenFileEnd;
			break;
		case '"':
	...
	bool Reader::readString(bool simple_string)
	{
		Char c = 0;
		bool bSourceString = ((simple_string == false) && (*current_ == '\''));
	...
	bool Reader::readObject( Token &tokenStart )
	{
		...
		while ( readToken( tokenName ) )
		{
			...
			if  ( !initialTokenOk )
				break;
			if ( tokenName.type_ == tokenObjectEnd )  // empty object
				return true;
	...
	bool Reader::loadFile( Token &token )
	{
		std::string name("");
		if ( !decodeString( token, name ) )
			return recoverFromError( tokenFileEnd );
		...
		{
			char szMessage[1024];
			sprintf( szMessage, "open file %s error", name.c_str() );
			return addErrorAndRecover( szMessage, token, tokenFileEnd );
		}

		Json::Reader reader;
		if( (ok = reader.parse( ifs, currentValue(), collectComments_ )) == false )
		{
			// printf( "parse file %s error\n %s\n", name.c_str(), reader.getFormatedErrorMessages().c_str() );
			currentValue().clear();
			return addErrorAndRecover( reader.getFormatedErrorMessages(), token, tokenFileEnd );
		}
	...
	{
		std::string decoded;
		if( *token.start_ == '"' && *(token.start_+1) == '\'' )
		{
			decoded.append( token.start_ + 2, token.end_ - 2 );
		}
		else if ( !decodeString( token, decoded ) )
		{
			return false;
		}
	...
	bool Reader::decodeString( Token &token, std::string &decoded )
	{
		...
		{
			decoded.reserve( token.end_ - token.start_ );
		}
