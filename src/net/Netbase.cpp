...
namespace xgc
{
	namespace net
	{
		...
		{
			asio_ServerBase *pServer = (asio_ServerBase*) server;
			if( pServer )
			{
				pServer->StopServer();
		...
		xgc_bool Connect( xgc_lpcstr address, xgc_uint16 port, xgc_uint16 timeout, INetworkSession* session )
		{
			asio_SocketPtr pSocket = std::make_shared< asio_Socket >( getNetwork().Ref(), session, 0 );

			if( xgc_nullptr == pSocket )
				return false;

			return pSocket->connect( address, port, false, timeout );
		}

		xgc_bool ConnectAsync( xgc_lpcstr address, xgc_uint16 port, xgc_uint16 timeout, INetworkSession* session )
		{
			asio_SocketPtr pSocket = std::make_shared< asio_Socket >( getNetwork().Ref(), session, 0 );
			...
			return pSocket->connect( address, port, true, timeout );
		}
