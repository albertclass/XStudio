...
#define EYESHOTAREA_FORCEfLUSH  256   ///< 视野强制刷新,9-16
	...
	public:
		typedef xgc_uint32 TriggerKey;
		typedef xgc_unordered_map< TriggerKey, XAction* > ActionMap;
		typedef xgc_unordered_map< xgc_string, ActionMap > SourceMap; ///< 场景触发器触发端
	...
	protected:
		...
		struct MapCell *mpCells;	///< 地砖数据
		struct IActorMapEventHandler *mpActorEventHandler; ///< 角色事件

		SourceMap	mSourceMap;

		MapCounter	mMapCounter; ///< 计数器配置
		MapSwitch	mMapSwitch;  ///< 场景内的开关对象
		MapTimer	mMapTimer;	 ///< 场景内的计时器
		MapClock	mMapClock;	 ///< 场景内的定时器

		xgc_vector< IBlockExtern* > mBlockExtern; ///< 地砖扩展信息

		/// @var 客户端事件列表 < event, state >
		xgc_unordered_map< xgc_uint16, xgc_uint16 > mClientEvents;

		/// @var 服务器事件列表 < event, eventHandle >
		xgc_unordered_map< xgc_uint16, function< xgc_void( XGameMap*, xgc_uint16, xgc_uint16 ) > > mServerEvents;
	...
	public:
		...
		{
			return GetAttribute( attrSceneFlags ).GetBit( (xgc_size) MapFlags::eCanMove, true );
		}
		...
		{
			if( bCanMove )
				GetAttribute( attrSceneFlags ).SetBit( (xgc_size) MapFlags::eCanMove );
			else
				GetAttribute( attrSceneFlags ).ClrBit( (xgc_size) MapFlags::eCanMove );
		}
		...
		///
		/// 获取触发器列表
		/// [7/19/2015] create by albert.xu
		///
		const SourceMap& GetMapTrigger()const
		{
			return mSourceMap;
		}
		...
		/// 创建闹钟
		///
		xgc_void CreateClock( xObject hSender, xgc_lpcstr lpName, xgc_lpcstr lpDate, xgc_lpcstr lpTime, xgc_lpcstr lpPass );

		///
		/// 创建计时器
		/// [7/23/2014] create by albert.xu
		///
		xgc_void CreateTimer( xObject hSender, xgc_lpcstr lpName, xgc_uint32 nRepeat, xgc_real32 fInterval, xgc_real32 fDelay = 0.0f );

		/////
		/// [12/22/2010 Albert]
		/// Description:	 插入触发器 
		/////
		xgc_void CreateTrigger( xgc_lpcstr lpSourceName, xgc_uint16 nTriggerEvent, xgc_uint16 nTriggerCode, XAction* pAction );
		...
		/// [7/28/2014] create by albert.xu
		///
		xgc_void DeleteTrigger( xgc_lpcstr lpSourceName, xgc_uint16 nClass, xgc_uint16 nEvent );

		/////
		/// 增加计数器 
		/// [1/5/2011 Albert]
		/////
		xgc_void IncCounter( xObject hSender, xgc_lpcstr lpCounterName, xgc_long nInc );

		/////
		/// 设置计数器 
		/// [1/5/2011 Albert]
		/////
		xgc_void SetCounter( xObject hSender, xgc_lpcstr lpCounterName, xgc_long nVal );

		///
		/// 获取计数器值
		/// [11/5/2014] create by albert.xu
		///
		xgc_bool GetCounter( xgc_lpcstr lpName, xgc_long &nValue );

		/////
		/// 转换开关
		/// [1/5/2011 Albert]
		/////
		xgc_void TurnSwitch( xObject hSender, xgc_lpcstr lpSwitchName, xgc_long nSwitch );
		...
		///
		const xgc_unordered_map< xgc_uint16, xgc_uint16 >& GetClientEvents()const;

		///
		/// 获取指定场景事件的值
		/// [4/3/2015] create by jianglei.kinly
		///
		xgc_uint16 GetClientEvent( xgc_uint16 )const;
	...
	XGC_INLINE xgc_void XGameMap::OutputStringMap( xgc_lpcstr lpFileName )
	{
		FILE* fp;
		if( lpFileName )
		{
	...
	XGC_INLINE xgc_void XGameMap::CreateCounter( xgc_lpcstr lpCounterName, xgc_long nInit, xgc_long nMax, xgc_long nMin )
	{
	...
	/// Description:	创建计数器 
	/////
	XGC_INLINE xgc_void XGameMap::CreateSwitch( xgc_lpcstr lpSwitchName, xgc_long nInit )
	{
		auto ib = mMapSwitch.insert( std::make_pair( lpSwitchName, nInit ) );
		XGC_ASSERT( ib.second );
	}

	XGC_INLINE xgc_void XGameMap::CreateTrigger( xgc_lpcstr lpSourceName, xgc_uint16 nTriggerEvent, xgc_uint16 nTriggerCode, XAction* pAction )
	{
		XGC_ASSERT_RETURN( lpSourceName && lpSourceName[0], xgc_void( 0 ) );
		auto &evtMap = mSourceMap[lpSourceName];
		evtMap[XGC_MAKEDW( nTriggerEvent, nTriggerCode )] = pAction;
	}
	...
	/// [7/28/2014] create by albert.xu
	///
	XGC_INLINE xgc_void XGameMap::DeleteTrigger( xgc_lpcstr lpSourceName, xgc_uint16 nClass, xgc_uint16 nEvent )
	{
		auto its = mSourceMap.find( lpSourceName );
		if( its == mSourceMap.end() )
			return;

		auto itc = its->second.find( XGC_MAKEDW( nClass, nEvent ) );
		if( itc == its->second.end() )
			return;

		its->second.erase( itc );

		if( its->second.empty() )
			mSourceMap.erase( its );
	}
	...
	XGC_INLINE xgc_bool XGameMap::TriggerClientEvent( xgc_uint16 nEvent, xgc_uint16 nState )
	{
		...
		XGC_ASSERT_RETURN( it != mClientEvents.end(), false, "Scene = %s, nEvent = %u", GetStrAttr( attrSceneTitle ), nEvent );
	...
	XGC_INLINE xgc_bool XGameMap::TriggerServerEvent( xgc_uint16 nEvent, xgc_uint16 nState )
	{
		...
		XGC_ASSERT_RETURN( it != mServerEvents.end(), false, "Scene = %s, nEvent = %u", GetStrAttr( attrSceneTitle ), nEvent );
	...
	/// [10/28/2014] create by albert.xu
	///
	XGC_INLINE const xgc_unordered_map< xgc_uint16, xgc_uint16 >& XGameMap::GetClientEvents() const
	{
	...
	/// [4/3/2015] create by jianglei.kinly
	///
	XGC_INLINE xgc_uint16 XGameMap::GetClientEvent( xgc_uint16 _key )const
	{
